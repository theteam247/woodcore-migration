woodcoin Core version 0.18.1 is now available from:
...
`/Applications/woodcoin-Qt` (on Mac) or `woodcoind`/`woodcoin-qt` (on
Linux).
...
woodcoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use woodcoin Core on unsupported systems.
...
10.10. Additionally, woodcoin Core does not yet change appearance when
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@woodcoin.org) the woodcoin Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 LOG),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in LOG per kilobyte, not litoshi per byte.
...
- A new document introduces woodcoin Core's BIP174 [Partially-Signed
  woodcoin Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent woodcoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  woodcoin Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when woodcoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of woodcoin Core, subsequently importing
...
- A new `woodcoin-wallet` tool is now distributed alongside woodcoin
...
This section describes planned changes to woodcoin Core that may affect
other woodcoin software and services.

- Since version 0.16.0, woodcoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with woodcoin Core 0.20 (expected about
  a year after 0.18), woodcoin Core will default to native segwit
  ...
  to bech32 addresses, and if the woodcoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in woodcoin Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any woodcoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, woodcoin Core
...
- A sub-project of woodcoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with woodcoin Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to woodcoin Core's own implementation, although entropy
  gathered by woodcoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves woodcoin Core a
...
- On macOS, woodcoin Core now opts out of application CPU throttling
